...
System: >        <form id="editMedicineForm">
            <label for="editMedicineName">ឈ្មោះថ្នាំ:</label>
...
// Function to initialize Firebase with hardcoded medicines only if the database is empty
async function initializeMedicines() {
    const medicinesRef = ref(db, 'medicines');
    const snapshot = await get(medicinesRef);

    // If the database is empty, initialize with hardcoded medicines
    if (!snapshot.exists()) {
        const medicinesToAdd = medicineOptions.filter(name => name && name.trim() !== "");
