...
function showGlobalNotePopup(recordId, visitId, itemId, rowElement) {
    ...
    if (!recordId || !visitId || !itemId) {
        popup.innerHTML = `<div class="global-note-popup-content"><span class="close-global-note-popup">×</span><p>Error: Missing parameters</p></div>`;
        finalizePopup();
        return;
    }
    ...
    get(noteRef).then(snapshot => {
        const globalNote = snapshot.exists() && typeof snapshot.val() === 'string' ? snapshot.val() : 'មិនមានកំណត់ចំណាំ';
        popup.innerHTML = `
...
window.onload = function () {
    ...
    if (recordId) {
        getPatientDetails(recordId, visitId).then(({ visits }) => {
            ...
            if (patientNotesContainer) {
                patientNotesContainer.addEventListener('click', (event) => {
                    ...
                    if (button) {
                        ...
                        if (itemId && recordId && effectiveVisitId) {
            ...
            document.getElementById('patientNotes').innerHTML = `<div class="error-message">Error: ${error.message}</div>`;
        });
    } else {
        console.error('No recordId found in URL');
        document.getElementById('patientNotes').innerHTML = `<div class="error-message">Error: No patient ID provided.</div>`;
    }
};