...
function parseDoseValue(dose) {
    ...
    // Handle fractions and compound values like "1+1/4"
    if (dose.includes('+')) {
...
function calculateMedicationQuantity(item) {
    ...
    const quantityInput = item.querySelectorAll('.time-input')[1];
    ...
    // Display result with decimal places only if needed
...
async function savePatientNotes(recordId) {
    ...
    document.querySelectorAll('.medicine-item').forEach(item => {
        medicines.push({
            ...
            quantity: item.querySelectorAll('.time-input')[1].value
        });
    });
    ...
    <table class="medicine-table">
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <input type="number" class="time-input" placeholder="ថ្ងៃ" min="1">
                </td>
                <td>
