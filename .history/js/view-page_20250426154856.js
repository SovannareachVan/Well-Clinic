...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        const addressContainer = document.getElementById('patientAddress');
        if (patientData.address) {
            ...
            addressContainer.textContent = addressParts.join(', ') || 'N/A';
        } else {
            addressContainer.textContent = 'N/A';
        }
        ...
        if (visitId && patientData.visits?.[visitId]) {
            ...
            visits.forEach(([visitId, visit], index) => {
                const isFirstVisit = index === 0; // first visit = detailed notes
                outputHtml += generateVisitHtml(
                    ...
                    visit.doctor,
                    visit.information || {},
                    isFirstVisit // true if first visit
                );
            });
        }
