    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            Object.entries(visits).forEach(([visitId, visit], index) => {
                ...
                const [checkInDate, checkInTime] = visit.checkIn.split(' ');
                checkInCell.innerHTML = `${checkInDate}<br>${checkInTime}`;

                // Check-out time
                const checkOutCell = newRow.insertCell(2);
                checkOutCell.innerHTML = visit.checkOut !== 'N/A' 
                    ? visit.checkOut.replace(' ', '<br>') 
                    : 'N/A';
...
async function saveAllRows() {
    ...
    for (let row of rows) {
        ...
        updates[`patients/${recordId}/visits/${visitId}`] = {
            checkIn: row.cells[1].innerText.replace('<br>', ' '),
            checkOut: row.cells[2].innerText.replace('<br>', ' '),
