...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        if (!snapshot.exists()) {
            showErrorMessage('No patient records available.');
            return;
        }
        ...
        // Show basic info
        setTextContent('patientFullName', patientData.fullName);
        setTextContent('patientAge', patientData.age);
        setTextContent('patientGender', patientData.gender);
        setTextContent('patientPhone', patientData.phone);
        
        // Show address information if it exists
        if (patientData.address) {
            const addressParts = [
                patientData.address.street,
                patientData.address.village ? `ភូមិ ${patientData.address.village}` : '',
                patientData.address.commune ? `ឃុំ/សង្កាត់ ${patientData.address.commune}` : '',
                patientData.address.district ? `ស្រុក/ខណ្ឌ ${patientData.address.district}` : '',
                patientData.address.province ? `ខេត្ត/ក្រុង ${patientData.address.province}` : ''
            ].filter(part => part !== '');
            
            setTextContent('patientAddress', addressParts.join(', '));
        } else {
            setTextContent('patientAddress', 'N/A');
        }

        const visits = patientData.visits ? Object.entries(patientData.visits) : [];
        let outputHtml = '';

        // Add patient notes at the top if they exist
        ...
        showErrorMessage('Failed to load patient details.');
    }
}
...
function showErrorMessage(message) {
    const errorContainer = document.getElementById('patientNotes') || document.body;
    errorContainer.innerHTML = `<div class="error-message">${message}</div>`;
}

// Rest of your existing functions (generateVisitHtml, generateFirstVisitContent, etc.) remain the same
