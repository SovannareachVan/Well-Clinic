...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        // Show address properly
        if (patientData.address) {
            const addressParts = [];
        
            if (patientData.address.village) addressParts.push(`ភូមិ ${patientData.address.village}`);
            if (patientData.address.commune) addressParts.push(`ឃុំ ${patientData.address.commune}`);
            if (patientData.address.district) addressParts.push(`ស្រុក ${patientData.address.district}`);
            if (patientData.address.province) addressParts.push(`ខេត្ត ${patientData.address.province}`);
        
            const addressString = addressParts.join(', ');
        
            const addressElement = document.getElementById('patientAddress');
            if (addressElement) {
                addressElement.textContent = addressString || 'N/A';
            }
        }
