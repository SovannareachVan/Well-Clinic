    ...
    async function getPatientDetails(recordId, visitId = null) {
        try {
            ...
            if (patientSnapshot.exists()) {
                ...
                // Display basic info
                document.getElementById('patientFullName').textContent = basicInfo.fullName;
                document.getElementById('patientAge').textContent = basicInfo.age;
                document.getElementById('patientGender').textContent = basicInfo.gender;
                document.getElementById('patientPhone').textContent = basicInfo.phone;
                document.getElementById('patientEmail').textContent = basicInfo.email || 'N/A';
                document.getElementById('patientName').textContent = basicInfo.fullName;
                
                // Handle notes display
                if (typeof basicInfo.notes === 'string') {
                    document.getElementById('patientNotes').textContent = basicInfo.notes;
                } else if (basicInfo.notes?.original) {
                    document.getElementById('patientNotes').textContent = basicInfo.notes.original;
                } else {
                    document.getElementById('patientNotes').textContent = '';
                }

                // Determine where to load structured notes from
    // Load structured notes from visit or root if not found
    if (visitId) {
    ...
    window.addMedicineItem = function (medicineData = null) {
        ...
        ul.appendChild(li);
        initMedicineDropdown(li); // Initialize dropdown if you have auto-complete
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // ✅ Always add one row even without recordId
        addMedicineItem();
