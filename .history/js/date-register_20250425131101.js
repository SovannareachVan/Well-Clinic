    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            Object.entries(visits).forEach(([visitId, visit], index) => {
                ...
                const checkInParts = visit.checkIn.split(' ');
                checkInCell.innerHTML = checkInParts.length >= 2 
                    ? `${checkInParts[0]}<br>${checkInParts[1]}` 
                    : visit.checkIn;

                // Check-out time
                const checkOutCell = newRow.insertCell(2);
                if (visit.checkOut !== 'N/A') {
                    const checkOutParts = visit.checkOut.split(' ');
                    checkOutCell.innerHTML = checkOutParts.length >= 2 
                        ? `${checkOutParts[0]}<br>${checkOutParts[1]}` 
                        : visit.checkOut;
                } else {
                ...
                clinicSelect.value = visit.clinic;
                clinicSelect.addEventListener('change', (e) => {
                    if (visit.clinic !== e.target.value) {
                        saveAllRows();
                    }
                });
