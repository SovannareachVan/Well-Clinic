...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        if (visitId) {
            ...
            const isFirstVisit = visits.findIndex(v => v[0] === visitId) === 0; // Newest visit (first in sorted array)
            ...
            for (const [currentVisitId, visit] of visits) {
                ...
                const visitNumber = totalVisits - visits.findIndex(v => v[0] === currentVisitId); // Newest gets highest number
...
function generateFirstVisitContent(info) {
    return `
        ...
        <div class="note-item"><strong>តេស្តមន្ទីពិសោធន៍:</strong> ${info.note3 || 'មိនទាន់បំពេញ'}</div>
