...
function initMedicineDropdown(parentElement) {
    ...
    // Fetch medicines from Firebase with real-time updates and debugging
    const medicinesRef = ref(db, `patients/${patientId}/medicines`);
    onValue(medicinesRef, (snapshot) => {
        medicineOptions = [];
        console.log('Firebase data updated for patientId:', patientId);
        if (snapshot.exists()) {
            ...
            console.log('Current medicineOptions:', medicineOptions);
        } else {
            console.log('No medicines found for patientId:', patientId);
        }
    ...
    function updateDropdown() {
        ...
        const filteredOptions = query
            ...
            : medicineOptions;
        console.log('Updating dropdown with filteredOptions:', filteredOptions);
        filteredOptions.forEach(option => {
            const div = document.createElement("div");
            div.classList.add("dropdown-item");
            div.textContent = option;
    ...
    input.addEventListener('click', function () {
        dropdown.style.display = 'block';
        updateDropdown(); // Ensure dropdown is populated with latest options
        console.log('Dropdown clicked, options:', medicineOptions);
    });
}
