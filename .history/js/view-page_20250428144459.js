...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        // Translate address parts to Khmer
        function translateAddressPart(part) {
            if (!part) return '';
            console.log('Translating:', part); // Log the part being translated
            return part
                .replace('Village', 'ភូមិ')
                .replace('Commune', 'ឃុំ')
                .replace('District', 'ស្រុក')
                .replace('Province', 'ខេត្ត');
        }

        // Process and show address (if available)
        if (patientData.address) {
            const { village, commune, district, province } = patientData.address;
            console.log('Address data:', patientData.address); // Log address data

            const addressParts = [
                translateAddressPart(village),
                translateAddressPart(commune),
                translateAddressPart(district),
                translateAddressPart(province)
            ...
            console.log('No address found for this patient'); // Log if there's no address
            const patientAddressElement = document.getElementById('patientAddress');
            if (patientAddressElement) {
