...
const medicineOptions = [
    "ACA 5mg", "Amithir 25mg", "Amitriptyline CPE 25mg", "Amitriptyline 25mg", "Arizap 10mg", "Asolan 0.5mg",
    "Avestalo 10mg", "Avestalo 5mg", "Bromark 150mg", "Chlorpromazine 100mg", "Clozapine 100mg", "Dezodone 50mg",
    "Diazepam 5mg", "DV-LEO", "DV-Lopram", "Euphytose", "Eziness 30mg", "Haloperidol 10mg", "Lamnet 25mg",
    "Lamnet 50mg", "Lamnet 100mg", "Lanzap 2.5mg", "Lanzap 5mg", "Lanzap 10mg", "Lumark 500mg", "Merlopam 0.5mg",
    "Merlopam 2mg", "Morcet 10mg", "MultiV", "Nortriptyline 25mg", "Perphenazine 8mg", "Persidal 2mg",
    "Phenobarbitale 100mg", "Phenobarbitale 50mg", "Polytanol 25mg", "Ratraline 50mg", "Rismek 2mg", "Sertaline 50mg",
    "Thioridazine 10mg", "Trihexyphenidule 8mg", "Valdoxan 25mg", "Carbamazepine 200mg", "Zoloft 50mg"
];
...
function initDiagnosisDropdown() {
    ...
    input.addEventListener('input', function () {
        ...
        dropdown.innerHTML = '';
        const filteredOptions = query ? diagnosisOptions.filter(option => option.toLowerCase().includes(query)) : diagnosisOptions;
...
function initMedicineDropdown(parentElement) {
    ...
    input.addEventListener('input', function() {
        const query = this.value.toLowerCase();
        dropdown.innerHTML = '';
        const filteredOptions = query ? medicineOptions.filter(option => option.toLowerCase().includes(query)) : medicineOptions;
        filteredOptions.forEach(option => {
            const div = document.createElement("div");
            div.classList.add("dropdown-item");
            div.textContent = option;
...
function displaySavedInfoInForm(info) {
    ...
    setInputValue('treatmentHistory', info.treatmentHistory !== "N/A" ? info.treatmentHistory : '');
    setInputValue('labTest', info.labTest !== "N/A" ? info.labTest : '');
    setInputValue('diagnosis', info.diagnosis || '');
...
function clearForm() {
    ...
    setInputValue('treatmentHistory');
    setInputValue('labTest');
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (patientId && visitId) {
        try {
            ...
            if (snapshot.exists()) {
                console.log('Found visit-specific data:', snapshot.val());
                displaySavedInfoInForm(snapshot.val());
            } else {
                console.log('No visit data found, checking initial registration for patient:', patientId);
                const patientRef = ref(db, `patients/${patientId}/structuredNotes`);
                const patientSnapshot = await get(patientRef);
                if (patientSnapshot.exists()) {
                    console.log('Found initial registration data:', patientSnapshot.val());
                    const structuredNotes = patientSnapshot.val();
                    const initialInfo = {
                        treatmentHistory: structuredNotes.note2 || "N/A",
                        labTest: structuredNotes.note3 || "N/A",
                        diagnosis: structuredNotes.note4 || '',
                        medicines: structuredNotes.medicines || [],
                        createdAt: new Date().toISOString()
                    };
                    displaySavedInfoInForm(initialInfo);
                } else {
                    const fullPatientRef = ref(db, `patients/${patientId}`);
                    const fullSnapshot = await get(fullPatientRef);
                    console.warn('No initial registration data found for patient:', patientId, 'Full patient data:', fullSnapshot.val());
                    alert('មិនមានទិន្នន័យការចុះឈ្មោះដំបូងសម្រាប់អ្នកជំងឺនេះទេ។ សូមចុះឈ្មោះអ្នកជំងឺជាមុនសិន។');
                    addMedicineItem(null, true);
                }
            }
        } catch (error) {
            console.error('Error loading patient information:', error);
            alert('កំហុសក្នុងការផ្ទុកព័ត៌មាន៖ ' + error.message);
            addMedicineItem(null, true);
        }
    } else {
        console.warn('Missing patientId or visitId in URL parameters:', { patientId, visitId });
        alert('សូមផ្តល់លេខសម្គាល់អ្នកជំងឺ និងលេខសម្គាល់ដំណើរទស្សនកិច្ច។');
        addMedicineItem(null, true);
    }
    
    const saveBtn = document.getElementById('saveBtn');
    if (saveBtn) saveBtn.addEventListener('click', savePatientInformation);
    else console.error('Element with id "saveBtn" not found');

    const clearBtn = document.getElementById('clearBtn');
    if (clearBtn) clearBtn.addEventListener('click', clearForm);
    else console.error('Element with id "clearBtn" not found');
