...
function combineDateTime(day, month, year, time) {
    // If any date part is missing, return null to indicate invalid date
...
function validateDateInputs(day, month, year) {
    ...
    if (isNaN(yearNum) || yearNum < 1900 || yearNum > new Date().getFullYear()) return false;
    return true;
}
...
function checkIn() {
    ...
    const { dateStr, timeStr } = formatDateTime(new Date()); // Use current date for default
    ...
    checkInCell.innerHTML = `
        <div class="date-inputs">
            <input type="text" class="day-input" value="${dateStr.split('/')[0]}" placeholder="DD" maxlength="2" size="2">/
            <input type="text" class="month-input" value="${dateStr.split('/')[1]}" placeholder="MM" maxlength="2" size="2">/
            <input type="text" class="year-input" value="${dateStr.split('/')[2]}" placeholder="YYYY" maxlength="4" size="4">
    ...
    // Save the new visit to Firebase with proper checkIn date
    const visitData = {
        checkIn: combineDateTime(dateStr.split('/')[0], dateStr.split('/')[1], dateStr.split('/')[2], timeStr) || 'N/A',
        checkOut: 'N/A',
...
function checkOutAction(row) {
    ...
    // Validate check-out date before proceeding
    ...
    if (!validateDateInputs(checkOutDay, checkOutMonth, checkOutYear)) {
        alert('Please fill in a valid Check-out date (DD/MM/YYYY) before checking out.');
        return;
    }
    ...
    const checkOutInputsUI = checkOutCell.querySelector('.date-inputs');
    const timeDisplay = checkOutInputsUI.querySelector('.time-display');
    ...
    for (let row of rows) {
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                // Only update date-time if all fields are valid; otherwise, retain existing
                const newCheckInDateTime = combineDateTime(checkInDay, checkInMonth, checkInYear, checkInTime);
                const newCheckOutDateTime = checkOutTime === 'N/A' ? 'N/A' : combineDateTime(checkOutDay, checkOutMonth, checkOutYear, checkOutTime);

                const updatedData = {
                    ...existingData,
                    checkIn: newCheckInDateTime !== null ? newCheckInDateTime : existingData.checkIn,
                    checkOut: newCheckOutDateTime !== null ? newCheckOutDateTime : existingData.checkOut,
