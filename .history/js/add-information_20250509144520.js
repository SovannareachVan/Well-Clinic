...
function initDiagnosisDropdown() {
    ...
    input.addEventListener('input', function () {
        ...
        const filtered = query
            ? diagnosisOptions.filter(opt => opt.toLowerCase().includes(query))
            : diagnosisOptions;
...
window.addMedicineItem = function(medicineData = null) {
    ...
    if (!ul) {
        console.error("Element with ID 'medicineList' not found");
        return;
    }
...
function initMedicineDropdown(parentElement) {
    ...
    input.addEventListener('input', function () {
        ...
        dropdown.innerHTML = '';
        const filtered = query
            ? medicineOptions.filter(opt => opt.toLowerCase().includes(query))
            : [...medicineOptions];
        filtered.forEach(option => {
            const div = document.createElement("div");
            div.classList.add("dropdown-item");
