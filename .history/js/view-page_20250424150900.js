...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        visits.forEach(([visitId, visit], index) => {
            const title = `ព័ត៌មានពិនិត្យលើកទី ${index + 1}`;
            if (index === 0) {
                // First visit - show full data (from add-detail-page)
                ...
                // Second visit - show simplified data (from add-information-page)
                outputHtml += generateSimplifiedVisitHtml(
                    title,
                    visit.checkIn,
                    visit.checkOut,
                    visit.clinic,
...
function generateSimplifiedVisitHtml(title, checkIn, checkOut, clinic, doctor, info) {
    return `
        <div class="visit-note">
            <div class="visit-note-header">
                <h3>${title}</h3>
            ...
            <div class="visit-note-content">
                <div class="note-item"><strong>ប្រវត្តិព្យាបាល:</strong> ${info.note1 || 'មិនទាន់បំពេញ'}</div>
                <div class="note-item"><strong>តេស្តមន្ទីពិសោធន៍:</strong> ${info.note2 || 'មិនទាន់បំពេញ'}</div>
                <div class="note-item"><strong>រោគវិនិច្ឆ័យ:</strong> ${info.note3 || ''}</div>
                <div class="note-item"><strong>របៀបប្រើប្រាស់ថ្នាំ:</strong></div>
                ${info.medicines?.length > 0 ? generateMedicineTable(info.medicines) : '<div class="medicine-empty">មិនទាន់បំពេញ</div>'}
            </div>
        </div>
