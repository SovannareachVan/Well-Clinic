...
async function savePatientNotes(patientId, visitId = null) {
    const medicines = Array.from(document.querySelectorAll('.medicine-item')).map(item => ({
        ...
        note: item.querySelector('.medicine-note-input')?.value || '' // Include the note
    }));
...
// Function to show note popup for a medicine item
function showNotePopup(medicineItem, existingNote = '') {
    const popup = document.createElement('div');
    popup.classList.add('note-popup');
    popup.innerHTML = `
        <div class="note-popup-content">
            <span class="close-note-popup" style="cursor: pointer; float: right; font-size: 20px;">×</span>
            <h3>កំណត់ចំណាំសម្រាប់ថ្នាំ</h3>
            <textarea class="medicine-note-input" style="width: 100%; height: 150px; resize: vertical;" placeholder="បញ្ចូលកំណត់ចំណាំនៅទីនេះ...">${existingNote}</textarea>
            <button class="save-note-btn" style="margin-top: 10px;">រក្សាទុក</button>
        </div>
    `;
    ...
    const closeBtn = popup.querySelector('.close-note-popup');
    closeBtn.onclick = () => {
        popup.remove();
    };
    ...
    document.addEventListener('click', function handleOutsideClick(event) {
        if (!popup.contains(event.target) && !event.target.classList.contains('note-icon')) {
            popup.remove();
    ...
    const noteInput = popup.querySelector('.medicine-note-input');
    saveBtn.onclick = () => {
        const noteText = noteInput.value.trim();
        // Store the note in a hidden input for persistence
        let noteInputElement = medicineItem.querySelector('.medicine-note-input');
        if (!noteInputElement) {
            noteInputElement = document.createElement('input');
            noteInputElement.type = 'hidden';
            noteInputElement.classList.add('medicine-note-input');
            medicineItem.appendChild(noteInputElement);
        }
        noteInputElement.value = noteText;
        popup.remove();
    };
}
...
window.addMedicineItem = function (medicineData = null) {
        ...
        <table class="medicine-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <button class="note-icon" style="background: none; border: none; cursor: pointer;" onclick="showNotePopup(this.closest('li'), this.closest('tr').querySelector('.medicine-note-input')?.value || '')">
                            📝
                        </button>
                    </td>
    ...
    if (medicineData) {
        ...
        if (medicineData.note) {
            const noteInput = document.createElement('input');
            noteInput.type = 'hidden';
            noteInput.classList.add('medicine-note-input');
            noteInput.value = medicineData.note;
