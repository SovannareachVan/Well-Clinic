...
const diagnosisOptions = [
    ...
    // ... (keep your existing diagnosis options)
];

const medicineOptions = [
    "Paracetamol 500mg",
    "Amoxicillin 500mg",
    // ... (keep your existing medicine options)
];
...
function parseDoseValue(dose) {
    ...
    if (dose.includes('+')) return dose.split('+').reduce((sum, part) => sum + parseDoseValue(part), 0);
    if (dose.includes('/')) {
...
async function loadMedicalInfo() {
    ...
    try {
        const medicalInfoRef = ref(db, `patients/${patientId}/visits/${visitId}/medicalInfo`);
        const snapshot = await get(medicalInfoRef);
        
        if (snapshot.exists()) {
            const data = snapshot.val();
            document.getElementById('treatmentHistory').value = data.treatmentHistory || '';
            document.getElementById('labTest').value = data.labTest || '';
            document.getElementById('diagnosis').value = data.diagnosis || '';
            
            document.getElementById('medicineList').innerHTML = '';
            if (data.medicines && data.medicines.length > 0) {
                data.medicines.forEach(med => addMedicineItem(med));
            } else {
                addMedicineItem();
            }
        } else {
...
async function savePatientInformation() {
    ...
    try {
        await set(ref(db, `patients/${patientId}/visits/${visitId}/medicalInfo`), patientInfo);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        ...
        if (diagnosisDropdown && diagnosisInput && !diagnosisInput.contains(event.target) {
