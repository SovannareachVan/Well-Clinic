...
// Function to position the dropdown relative to the viewport
function positionDropdown(input, dropdown) {
    const rect = input.getBoundingClientRect();
    const dropdownHeight = dropdown.offsetHeight || 300; // Estimate if not visible yet
...
function initMedicineDropdown(parentElement) {
    ...
    input.addEventListener('input', function (e) {
        ...
        if (filteredOptions.length) {
            ...
            dropdown.style.display = 'block';
            // Add scroll event listener to update position on page scroll
            if (!scrollHandler) {
                scrollHandler = () => {
