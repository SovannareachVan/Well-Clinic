    ...
    try {
        ...
        if (patientData.address) {
            ...
            if (typeof patientData.address === 'object') {
                ...
                ].filter(Boolean).join('<br>'); // Separate with line breaks
            }
            // For legacy string format
            else if (typeof patientData.address === 'string') {
                // Convert "Village 3, Commune 3..." to Khmer
                addressString = patientData.address
                    .replace('Village 1', 'ភូមិ ទួលក្របៅ')
                    .replace('Village 2', 'ភូមិ សាមកុក')
                    .replace('Village 3', 'ភូមិ ហាបី')
                    .replace('Commune 1', 'ឃុំ គគីរ')
                    .replace('Commune 2', 'ឃុំ កាស')
                    .replace('Commune 3', 'ឃុំ ក្អែក')
                    .replace('District 1', 'ស្រុក កៀនស្វាយ')
                    .replace('District 2', 'ស្រុក ក្អែក')
                    .replace('District 3', 'ស្រុក កាស')
                    .replace('Province 1', 'ខេត្ត ព្រៃវែង')
                    .replace('Province 2', 'ខេត្ត កណ្តាល')
                    .replace('Province 3', 'ខេត្ត ក្អាត់')
            ...
            if (patientAddressElement) {
                patientAddressElement.innerHTML = addressString || 'N/A';
            }
        }
