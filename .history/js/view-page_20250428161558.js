...
if (patientData.address) {
    ...
    // Check if the address is an object and has valid values
    const { village, commune, district, province } = patientData.address;

    // Only append non-empty fields
    const addressParts = [
        village ? `ភូមិ ${village}` : '', // Village label
        commune ? `ឃុំ ${commune}` : '', // Commune label
        district ? `ស្រុក ${district}` : '', // District label
        province ? `ខេត្ត ${province}` : '' // Province label
    ].filter(Boolean); // Remove empty strings

    // Join the address parts with line breaks for display
    addressString = addressParts.join('\n'); // Newline separator for formatting
