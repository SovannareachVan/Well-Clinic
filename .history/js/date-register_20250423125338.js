        ...
        if (snapshot.exists()) {
            ...
            Object.entries(visits).forEach(([visitId, visit], index) => {
                ...
                viewBtn.addEventListener('click', () => {
                    // Check if this is the first visit (index 0) AND there are no other visits
                    const isFirstVisit = index === 0 && Object.keys(visits).length === 1;
                    
                    if (isFirstVisit) {
                        // For first check-in, go to add-detail-page.html
                        window.location.href = `add-detail-page.html?patientId=${recordId}&visitId=${visitId}`;
                    } else {
                        // For all other check-ins, go to add-information.html
