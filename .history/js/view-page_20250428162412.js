...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        if (patientData.address) {
            ...
            // Map the Firebase values to the actual labels
            const addressParts = [
                village ? `ភូមិ ${villageMap[village] || village}` : '', // Use mapped village name or the raw value if not found
                commune ? `ឃុំ ${communeMap[commune] || commune}` : '', // Same for commune
                district ? `ស្រុក ${districtMap[district] || district}` : '', // Same for district
                province ? `ខេត្ត ${provinceMap[province] || province}` : '' // Same for province
            ].filter(Boolean); // Remove any empty strings

            // Join the parts into one string
            addressString = addressParts.join(', '); // Add commas between the address parts
