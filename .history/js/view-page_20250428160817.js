    ...
    try {
        ...
        if (patientData.address) {
            ...
            if (typeof patientData.address === 'object') {
                // Directly use the Khmer values stored in the address object
                addressString = [
            ...
            // For legacy string format (English values)
            else if (typeof patientData.address === 'string') {
                // Map English values to Khmer
                const addressMap = {
                    // Villages
                    'Village 1': 'ភូមិ ទួលក្របៅ',
                    'Village 2': 'ភូមិ សាមកុក',
                    'Village 3': 'ភូមិ ហាបី',
                    // Communes
                    'Commune 1': 'ឃុំ គគីរ',
                    'Commune 2': 'ឃុំ កាស',
                    'Commune 3': 'ឃុំ ក្អែក',
                    // Districts
                    'District 1': 'ស្រុក កៀនស្វាយ',
                    'District 2': 'ស្រុក ក្អែក',
                    'District 3': 'ស្រុក កាស',
                    // Provinces
                ...
                // Convert each part of the address
                const parts = patientData.address.split(', ').map(part => addressMap[part] || part);
                addressString = parts.join('\n');
            }

            const patientAddressElement = document.getElementById('patientAddress');
            if (patientAddressElement) {
                // Use textContent and CSS white-space to handle newlines
                patientAddressElement.textContent = addressString || 'N/A';
                patientAddressElement.style.whiteSpace = 'pre-line';
            }
        } else {
            const patientAddressElement = document.getElementById('patientAddress');
            if (patientAddressElement) {
