        ...
        if (snapshot.exists()) {
            ...
            visits.forEach((visit, index) => {
                ...
                addBtn.addEventListener('click', () => {
                    const visitId = newRow.dataset.visitId || 'new'; // Use the row's visit ID
...
async function saveAllRows() {
    ...
    for (let row of rows) {
        const visitId = row.dataset.visitId || push(ref(db, 'patients/' + recordId + '/visits')).key; // Generate new ID if doesn't exist
        ...
        visits[visitId] = { 
            ...
            clinic, 
            doctor,
            visitId // Store the ID with the visit data
        };
        row.dataset.visitId = visitId; // Store the ID on the row
    }
