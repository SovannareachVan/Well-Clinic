...
import { ref, get, push, update  } from 'https://www.gstatic.com/firebasejs/9.6.1/firebase-database.js';
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            Object.entries(visits).forEach(([visitId, visit], index) => {
                ...
                clinicSelect.addEventListener('change', saveAllRows); // Auto-save on clinic change
...
function checkIn() {
    ...
    // Auto-save after adding a new check-in
    saveAllRows();
}
...
function checkOutAction(row) {
    ...
    // Auto-save after checkout
    saveAllRows();
}

import { ref, get, push, update, remove } from 'https://www.gstatic.com/firebasejs/9.6.1/firebase-database.js'; // make sure `remove` is imported
