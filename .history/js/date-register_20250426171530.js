...
// Fetch patient details and check if first visit
async function getPatientDetails(id) {
    try {
        ...
        if (patientSnapshot.exists()) {
            ...
            // Check if each element exists before setting the textContent
            const elements = {
                patientName: document.getElementById('patientName'),
                patientFullName: document.getElementById('patientFullName'),
                ...
                patientVillage: document.getElementById('village'),
                patientCommune: document.getElementById('commune'),
                patientDistrict: document.getElementById('district'),
                patientProvince: document.getElementById('province')
            };

            // Update patient info in DOM
            if (elements.patientName) elements.patientName.textContent = patientData.fullName || 'N/A';
            if (elements.patientFullName) elements.patientFullName.textContent = patientData.fullName || 'N/A';
            if (elements.patientAge) elements.patientAge.textContent = patientData.age || 'N/A';
            if (elements.patientGender) elements.patientGender.textContent = patientData.gender || 'N/A';
            if (elements.patientPhone) elements.patientPhone.textContent = patientData.phone || 'N/A';
            if (elements.patientEmail) elements.patientEmail.textContent = patientData.email || 'N/A';
            if (elements.patientNotes) elements.patientNotes.textContent = patientData.notes || 'No notes available.';

            // Address: Populate address fields if available
            if (patientData.address) {
                if (elements.patientVillage) elements.patientVillage.value = patientData.address.village || '';
                if (elements.patientCommune) elements.patientCommune.value = patientData.address.commune || '';
                if (elements.patientDistrict) elements.patientDistrict.value = patientData.address.district || '';
                if (elements.patientProvince) elements.patientProvince.value = patientData.address.province || '';
            }
