...
window.addMedicineItem = function (medicineData = null) {
        ...
        <table class="medicine-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <button class="note-icon" style="background: none; border: none; cursor: pointer; margin-right: 5px;" onclick="showNotePopup(this.closest('li'), this.closest('tr').querySelector('.medicine-note-input')?.value || '')">
                            📝
                        </button>
                        <button class="btn-delete" style="background: none; border: none; cursor: pointer;" onclick="this.closest('li').remove()">
                            ❌
                        </button>
                    </td>
                </tr>
    ...
    // Add a hidden input to store the note for this medicine item
...
window.showNotePopup = function (medicineItem, existingNote = '') {
    // Create the popup element
    const popup = document.createElement('div');
    popup.classList.add('note-popup');
    popup.innerHTML = `
        <div class="note-popup-content">
            <span class="close-note-popup">×</span>
            <h3>កំណត់ចំណាំសម្រាប់ថ្នាំ</h3>
            <textarea class="medicine-note-input-textarea" placeholder="បញ្ចូលកំណត់ចំណាំនៅទីនេះ...">${existingNote}</textarea>
            <button class="save-note-btn">រក្សាទុក</button>
        </div>
    `;
    ...
    const noteTextarea = popup.querySelector('.medicine-note-input-textarea');
    saveBtn.addEventListener('click', () => {
        const noteText = noteTextarea.value.trim();
        // Store the note in a hidden input
        let noteInput = medicineItem.querySelector('.medicine-note-input');
        if (!noteInput) {
            noteInput = document.createElement('input');
            noteInput.type = 'hidden';
            noteInput.classList.add('medicine-note-input');
            medicineItem.appendChild(noteInput);
        }
...
styleSheet.textContent = `
    ...
    .medicine-note-input-textarea:focus {
        outline: none;
        border-color: #007bff;
