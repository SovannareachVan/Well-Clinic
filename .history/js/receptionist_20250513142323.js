...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ['commune', 'district', 'province'].forEach(type => {
        ...
        inputField.addEventListener('input', e => {
            const value = e.target.value.trim().toLowerCase(); // case-insensitive search
            let data;
            if (type === 'commune') data = commune;
            if (type === 'district') data = district;
            if (type === 'province') data = province;
            
            const filtered = data.filter(item => item.toLowerCase().includes(value)); // case-insensitive filtering
            suggestionList.innerHTML = '';
