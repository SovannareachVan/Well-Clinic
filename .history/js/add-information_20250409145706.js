...
async function getPatientDetails(recordId) {
    try {
        ...
        if (snapshot.exists()) {
            ...
            const fullNameElement = document.getElementById('patientFullName');
            ...
            const notesElement = document.getElementById('patientNotes');

            if (fullNameElement) fullNameElement.textContent = patientData.fullName;
            if (ageElement) ageElement.textContent = patientData.age;
            if (genderElement) genderElement.textContent = patientData.gender;
            if (phoneElement) phoneElement.textContent = patientData.phone;
            if (emailElement) emailElement.textContent = patientData.email || 'N/A';
            if (notesElement) notesElement.textContent = patientData.notes || '';
...
async function savePatientNotes(recordId) {
    ...
    try {
        // Save data to Firebase without alert
        await update(ref(db, 'patients/' + recordId), { 
            structuredNotes: structuredNotes
        });

        // Instead of alert, just log success
        console.log('Notes saved successfully!');
        
        // Avoid reloading patient data
        // Optionally, remove the call to getPatientDetails(recordId) if not needed after save
        // await getPatientDetails(recordId); // You can comment this out if you don't want to reload data

        // Optionally, go back to the previous page
        window.history.back(); // Go back to the previous page instead of redirecting
    } catch (error) {
        console.error('Error saving patient notes:', error);
        // Remove alert and just log the error
        console.error('Failed to save notes. Error: ' + error.message);
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (recordId) {
        // ❌ Remove or comment this line to prevent data fetching from Firebase
