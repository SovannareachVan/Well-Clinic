...
// Function to populate the dropdown options dynamically for note4
function filterOptions() {
    const note4Options = ["Cat", "Car", "Apple", "Fruit", "Banana", "Grapes", "Orange"];
    ...
    if (query) {
        const filteredOptions = note4Options.filter(option => option.toLowerCase().includes(query));
...
// Show dropdown when clicking the input box, and filter as the user types
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (recordId) {
        ...
        note4Input.addEventListener('input', filterOptions);
    } else {
