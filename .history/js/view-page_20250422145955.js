    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            if (patientData.visits) {
                ...
                // Sort visits by check-in date (OLDEST first - reverse of your current sort)
                visits.sort((a, b) => new Date(a[1].checkIn) - new Date(b[1].checkIn));
                
                visits.forEach(([visitId, visit], index) => {
                    const visitInfo = visit.information || {};
                    const visitNumber = (visits.length - index) + 1;// This will now show 3 first, then 2
