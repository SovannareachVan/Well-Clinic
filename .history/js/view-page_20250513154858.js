...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        const fields = [
            ...
            { id: 'patientTelegram', key: 'telegram' } // Add Telegram field
        ];
        ...
        // Sort visits by checkIn date (newest first), with error handling
        visits.sort((a, b) => {
            const dateA = a[1].checkIn && a[1].checkIn !== 'N/A' ? new Date(a[1].checkIn) : new Date(0);
            const dateB = b[1].checkIn && b[1].checkIn !== 'N/A' ? new Date(b[1].checkIn) : new Date(0);
...
function generateVisitHtml(title, checkIn, checkOut, clinic, doctor, info, isFirstVisit = false) {
    ...
    // Ensure checkIn and checkOut are strings, handle 'N/A' or invalid dates
    const checkInDisplay = checkIn && checkIn !== 'N/A' ? checkIn : 'N/A';
    const checkOutDisplay = checkOut && checkOut !== 'N/A' ? checkOut : 'N/A';

    // Fallback to prevent empty meta data from breaking layout
    const safeCheckIn = checkInDisplay === 'N/A' ? 'N/A' : checkInDisplay.split(' ')[0]; // Show only date if available
    const safeCheckOut = checkOutDisplay === 'N/A' ? 'N/A' : checkOutDisplay.split(' ')[0]; // Show only date if available
        ...
        <div class="visit-note">
            <div class="visit-note-header">
                ...
                <div class="visit-meta">
                    <div><strong>ថ្ងៃចូល:</strong> ${safeCheckIn}</div>
                    <div><strong>ថ្ងៃចេញ:</strong> ${safeCheckOut}</div>
