...
// Add this inside your addMedicineItem() function after creating the table HTML
const timeInput = li.querySelector('.time-input');
const morningSelect = li.querySelectorAll('.dosage-select')[1]; // First select after medicine type
const afternoonSelect = li.querySelectorAll('.dosage-select')[2]; // Second select
const eveningSelect = li.querySelectorAll('.dosage-select')[3]; // Third select
const quantityInput = li.querySelectorAll('.time-input')[1]; // Second input (quantity)

function calculateQuantity() {
    const days = parseFloat(timeInput.value) || 0;
    const morningDose = parseDose(morningSelect.value);
    const afternoonDose = parseDose(afternoonSelect.value);
    const eveningDose = parseDose(eveningSelect.value);
    
    const totalPerDay = morningDose + afternoonDose + eveningDose;
    const totalQuantity = days * totalPerDay;
    
    quantityInput.value = totalQuantity % 1 === 0 ? totalQuantity : totalQuantity.toFixed(1);
}

function parseDose(doseString) {
    if (!doseString) return 0;
    if (doseString.includes('+')) {
        const parts = doseString.split('+');
        return parseFloat(parts[0]) + parseFloat(parts[1]);
    }
    return parseFloat(doseString);
}

// Add event listeners
timeInput.addEventListener('input', calculateQuantity);
