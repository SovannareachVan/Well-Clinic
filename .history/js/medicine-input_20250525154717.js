...
function displayMedicineList() {
    ...
    onValue(medicinesRef, (snapshot) => {
        ...
        let medicines = [...medicineOptions]; // Start with default options from medicine-dropdown.js
        ...
        medicines = [...new Set(medicines)].filter(name => name !== ''); // Exclude empty string
        ...
        medicines.forEach((medicine, index) => {
            ...
            row.innerHTML = `
                <td>${index + 1}</td>
                <td>${medicine}</td>
                <td><button class="btn-delete" data-medicine="${medicine}" onclick="deleteMedicine('${medicine.replace(/'/g, "\\'")}', '${patientId}')">❌</button></td>
            `;
