...
function combineDateTime(day, month, year, time) {
    if (!day || !month || !year) return ` ${time}`; // If date parts are missing, store only the time
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            Object.entries(visits).forEach(([visitId, visit], index) => {
                ...
                checkOutBtn.className = 'btn ' + (visit.checkOut === '' || visit.checkOut === 'N/A' ? 'btn-checkout' : 'btn-disabled');
                checkOutBtn.textContent = visit.checkOut === '' || visit.checkOut === 'N/A' ? 'Check-out' : 'Checked-out';
                checkOutBtn.disabled = visit.checkOut !== '' && visit.checkOut !== 'N/A';
    ...
    for (let row of rows) {
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                const checkOutDateTime = checkOutTime === 'N/A' ? '' : combineDateTime(checkOutDay, checkOutMonth, checkOutYear, checkOutTime);
