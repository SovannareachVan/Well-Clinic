...
function showTotalPricePopup() {
    console.log("Total Price button clicked!"); // Debugging log

    const medicineItems = document.querySelectorAll('.medicine-item');
    console.log(`Found ${medicineItems.length} medicine items`); // Debugging log
    ...
    medicineItems.forEach((item, index) => {
        const name = item.querySelector('.medicine-input').value || `Medicine ${index + 1}`;
    ...
    okButton.onclick = () => {
        ...
        console.log(`Updated total: ${total.value}`); // Debugging log
        popup.remove();
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (patientId) {
        ...
        const totalPriceBtn = document.querySelector('.total-price-section button');
        if (totalPriceBtn) {
            console.log("Total Price button found, attaching event listener"); // Debugging log
            totalPriceBtn.addEventListener('click', showTotalPricePopup);
        } else {
            console.error("Total Price button not found in the DOM!");
        }
    }
