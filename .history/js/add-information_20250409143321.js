...
async function savePatientNotes(recordId) {
    ...
    try {
        ...
        // Wait a bit before refreshing the data
        setTimeout(async () => {
            await getPatientDetails(recordId); // reload data after save
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (recordId) {
        getPatientDetails(recordId); // Fetch patient details
        initDiagnosisDropdown(); // Initialize dropdown for diagnosis

        // Attach event listener to Save button
        document.getElementById('saveBtn').addEventListener('click', function () {
            savePatientNotes(recordId); // Save the notes when button is clicked
        });

        // Ensure at least one medicine item is added if the list is empty
        const ul = document.getElementById('medicineList');
        if (ul && ul.children.length === 0) {
            addMedicineItem(); // Add a medicine item if none exist
        }
    }
});
