...
// Function to initialize medicine dropdown with proper positioning
...
// Helper function to position the dropdown with fixed positioning
function positionDropdown(input, dropdown) {
    // Reset position to avoid CSS fallback interference
    dropdown.style.top = '0px';
    dropdown.style.left = '0px';
    dropdown.style.opacity = '0'; // Hide until positioned

    // Get the input's bounding rectangle
    const inputRect = input.getBoundingClientRect();
    const dropdownHeight = 300; // Max height of the dropdown (as set in CSS)
    const viewportHeight = window.innerHeight;
    const viewportWidth = window.innerWidth;
    const scrollX = window.scrollX;
    const scrollY = window.scrollY;

    // Calculate the absolute position of the input relative to the document
    let inputTop = inputRect.top + scrollY;
    let inputBottom = inputRect.bottom + scrollY;
    let inputLeft = inputRect.left + scrollX;

    // Account for any parent container scroll offsets
    let parent = input.parentElement;
    while (parent && parent !== document.body) {
        inputTop += parent.scrollTop || 0;
        inputLeft += parent.scrollLeft || 0;
        parent = parent.parentElement;
    }

    inputBottom = inputTop + inputRect.height;

    // Calculate available space relative to the viewport
    const spaceBelow = viewportHeight - (inputRect.bottom - scrollY);
    const spaceAbove = inputRect.top;

    // Set the width to match the input
    dropdown.style.width = `${inputRect.width}px`;

    // Debug logging to verify coordinates
    console.log('Input Rect:', inputRect);
    console.log('ScrollY:', scrollY, 'ScrollX:', scrollX);
    console.log('Adjusted Input Top:', inputTop, 'Input Bottom:', inputBottom, 'Input Left:', inputLeft);
    console.log('Space Below:', spaceBelow, 'Space Above:', spaceAbove);

    // Prioritize positioning below the input
    if (spaceBelow >= dropdownHeight) {
        dropdown.style.top = `${inputBottom}px`;
        dropdown.style.bottom = 'auto';
        dropdown.style.left = `${inputLeft}px`;
    } else if (spaceAbove >= dropdownHeight) {
        // Move above the input if not enough space below
        dropdown.style.top = `${inputTop - dropdownHeight}px`;
        dropdown.style.bottom = 'auto';
        dropdown.style.left = `${inputLeft}px`;
    } else {
        // If neither has enough space, position below and allow internal scrolling
        dropdown.style.top = `${inputBottom}px`;
        dropdown.style.bottom = 'auto';
        dropdown.style.left = `${inputLeft}px`;
        console.warn('Dropdown height exceeds available space; internal scrolling will be used.');
    }

    // Ensure the dropdown doesn’t go off-screen horizontally
    if (inputLeft + inputRect.width > viewportWidth) {
        dropdown.style.left = `${viewportWidth - inputRect.width}px`;
    }

    // Apply the position and show the dropdown
    setTimeout(() => {
        dropdown.style.opacity = '1'; // Fade in after positioning
        console.log('Final Dropdown Top:', dropdown.style.top, 'Final Dropdown Left:', dropdown.style.left);
