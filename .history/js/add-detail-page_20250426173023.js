...
async function getPatientDetails(recordId, visitId = null) {
    try {
        ...
        if (patientSnapshot.exists()) {
            ...
            // Display basic info
            document.getElementById('patientFullName').textContent = basicInfo.fullName || 'N/A';
            document.getElementById('patientAge').textContent = basicInfo.age || 'N/A';
            document.getElementById('patientGender').textContent = basicInfo.gender || 'N/A';
            document.getElementById('patientPhone').textContent = basicInfo.phone || 'N/A';
            document.getElementById('patientEmail').textContent = basicInfo.email || 'N/A';
            document.getElementById('patientName').textContent = basicInfo.fullName || 'N/A';
            
            // Display address if exists
            if (basicInfo.address) {
                const addressParts = [
                    basicInfo.address.street,
                    basicInfo.address.village ? `ភូមិ ${basicInfo.address.village}` : '',
                    basicInfo.address.commune ? `ឃុំ/សង្កាត់ ${basicInfo.address.commune}` : '',
                    basicInfo.address.district ? `ស្រុក/ខណ្ឌ ${basicInfo.address.district}` : '',
                    basicInfo.address.province ? `ខេត្ត/ក្រុង ${basicInfo.address.province}` : ''
                ].filter(part => part !== '');
                
                document.getElementById('patientAddress').textContent = addressParts.join(', ') || 'N/A';
            } else {
            ...
            if (typeof basicInfo.notes === 'string') {
                document.getElementById('patientNotes').value = basicInfo.notes;
            } else if (basicInfo.notes?.original) {
                document.getElementById('patientNotes').value = basicInfo.notes.original;
            } else {
                document.getElementById('patientNotes').value = '';
            }

            // Determine where to load structured notes from
            if (visitId) {
            ...
            ul.innerHTML = '';
            if (structuredNotes.medicines && structuredNotes.medicines.length > 0) {
                structuredNotes.medicines.forEach(med => {
                    const li = addMedicineItem(med);
                    calculateMedicationQuantity(li);
                });
            } else {
                // Add an empty medicine row if none exist
                addMedicineItem();
            }
        } else {
            console.error('Patient record not found');
            // Initialize with empty medicine row
            addMedicineItem();
        }
    } catch (error) {
        console.error('Error fetching patient details:', error);
        // Initialize with empty medicine row even if error occurs
        addMedicineItem();
    }
}
...
async function savePatientNotes(recordId, visitId = null) {
    ...
    })).filter(med => med.name); // Only include medicines with names
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (recordId) {
        ...
        // Initialize with empty medicine row if no patientId
        addMedicineItem();
    }
