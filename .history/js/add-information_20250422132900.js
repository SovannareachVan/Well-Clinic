...
function initDiagnosisDropdown() {
    ...
    const savedContainer = document.getElementById('savedDiagnosis');

    // Load saved diagnoses from localStorage if they exist
    let savedDiagnoses = JSON.parse(localStorage.getItem('savedDiagnoses')) || [];

    // Display any existing saved diagnoses
    updateSavedDiagnosesDisplay();

    if (!input || !dropdown || !savedContainer) return;
    ...
    input.addEventListener('input', function() {
        ...
        filteredOptions.forEach(option => {
            ...
            div.onclick = function() {
                // Add to saved diagnoses if not already there
                if (!savedDiagnoses.includes(option)) {
                    savedDiagnoses.push(option);
    ...
    function updateSavedDiagnosesDisplay() {
        ...
        savedDiagnoses.forEach((diagnosis, index) => {
            const diagnosisElement = document.createElement('div');
            diagnosisElement.classList.add('saved-item');
            
            const textSpan = document.createElement('span');
            textSpan.textContent = diagnosis;
            ...
            removeBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                savedDiagnoses.splice(index, 1);
                localStorage.setItem('savedDiagnoses', JSON.stringify(savedDiagnoses));
                updateSavedDiagnosesDisplay();
            });
            
            diagnosisElement.appendChild(textSpan);
            diagnosisElement.appendChild(removeBtn);
            savedContainer.appendChild(diagnosisElement);
        });
    }
}

// Initialize when DOM is loaded
document.addEventListener('DOMContentLoaded', initDiagnosisDropdown);
// Medicine options
