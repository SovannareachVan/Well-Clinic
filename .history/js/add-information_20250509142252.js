...
window.addMedicineItem = function (medicineData = null, forceAdd = false) {
    ...
    function calculateQuantity() {
        ...
        const totalQuantity = days * totalPerDay;
        li.querySelector('.quantity-input').value = totalQuantity % 1 === 0 ? totalQuantity : totalQuantity.toFixed(1);
    }
...
async function savePatientInformation() {
    ...
    document.querySelectorAll('.medicine-item').forEach(item => {
        ...
        if (name && !medicineNames.has(name)) {
    ...
    try {
        ...
        await set(visitRef, patientInfo);
