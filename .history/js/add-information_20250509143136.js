...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (patientId && visitId) {
        try {
            // First, try to load visit-specific data
            ...
            if (snapshot.exists()) {
                ...
                // If no visit-specific data, load initial registration data
                ...
                if (patientSnapshot.exists()) {
                    ...
                    // Fallback: Check if data was saved to the visit path (for compatibility with old data)
                    console.log('No structuredNotes found, checking visit path:', `patients/${patientId}/visits/${visitId}/information`);
                    ...
                    if (visitSnapshot.exists()) {
                        console.log('Found data in visit path:', visitSnapshot.val());
                        const visitData = visitSnapshot.val();
                        const initialInfo = {
                            treatmentHistory: "N/A",
                            labTest: "N/A",
                            diagnosis: visitData.note4 || '',
                            medicines: visitData.medicines || [],
                            createdAt: new Date().toISOString()
                        };
                        displaySavedInfoInForm(initialInfo);
                    } else {
                        // Debug: Check entire patient node
