...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- VILLAGE AUTOCOMPLETE START ---
    const villageInput = document.getElementById('village');
    const villageSuggestionList = document.getElementById('village-suggestions');
    let savedVillages = [];

    get(child(ref(db), 'villageSuggestions')).then(snapshot => {
        if (snapshot.exists()) {
            savedVillages = Object.keys(snapshot.val()).map(v => v.replace(/_/g, '.'));
        }
    }).catch(error => {
    ...
    villageInput.addEventListener('focus', () => {
        showVillageSuggestions(villageInput.value.trim().toLowerCase());
    });
    
    villageInput.addEventListener('input', e => {
        const value = e.target.value.trim().toLowerCase();
        showVillageSuggestions(value);
    });
    
    function showVillageSuggestions(value) {
    ...
    document.addEventListener('click', (e) => {
        if (!villageInput.contains(e.target) && !villageSuggestionList.contains(e.target)) {
            villageSuggestionList.style.display = 'none';
        }
    });
    ...
    patientForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            // Save new village for suggestions
            const villageKey = village.replace(/\./g, "_");
            const villageRef = ref(db, 'villageSuggestions/' + villageKey);
            await set(villageRef, true);
