...
async function getPatientDetails(recordId, visitId = null) {
    ...
    try {
        ...
        // If specific visitId is given in URL, show that one
        if (visitId && patientData.visits?.[visitId]) {
            const visit = patientData.visits[visitId];
            outputHtml = generateVisitHtml('ព័ត៌មានពិនិត្យ', visit.checkIn, visit.checkOut, visit.clinic, visit.doctor, visit.information || {});
        } else {
            // Show all visits
// Show the latest visit only
const [latestVisitId, latestVisit] = visits[0]; // already sorted
outputHtml = generateVisitHtml(
    'ព័ត៌មានពិនិត្យចុងក្រោយ',
    latestVisit.checkIn,
    latestVisit.checkOut,
    latestVisit.clinic,
    latestVisit.doctor,
    latestVisit.information || {}
);
