...
async function getPatientDetails(recordId) {
    try {
        ...
        if (snapshot.exists()) {
            ...
            // Get visit information if exists
            let visitInfoHtml = '';
            if (patientData.visits) {
                // Get the most recent visit
                const visits = Object.values(patientData.visits);
                const mostRecentVisit = visits[visits.length - 1]; // Assuming visits are ordered by date
                
                if (mostRecentVisit.information) {
                    const visitInfo = mostRecentVisit.information;
                    
                    // Create medicine list HTML from visit information
