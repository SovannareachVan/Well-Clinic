        ...
        async function savePatientInformation() {
            ...
            try {
                ...
                localStorage.setItem('lastSavedPatientInfo', JSON.stringify(patientInfo));
        ...
        function displaySavedInfoInForm(info) {
            ...
            const savedTreatmentHistory = document.getElementById('savedTreatmentHistory');
            savedTreatmentHistory.textContent = info.treatmentHistory;
            savedTreatmentHistory.style.display = info.treatmentHistory && info.treatmentHistory !== "N/A" ? 'block' : 'none';
            
            // Display lab test
            const savedLabTest = document.getElementById('savedLabTest');
            savedLabTest.textContent = info.labTest;
            savedLabTest.style.display = info.labTest && info.labTest !== "N/A" ? 'block' : 'none';
            
            // Display diagnosis
            const savedDiagnosis = document.getElementById('savedDiagnosis');
            savedDiagnosis.textContent = info.diagnosis;
            savedDiagnosis.style.display = info.diagnosis ? 'block' : 'none';
            
            // Display medicines
            const savedMedicines = document.getElementById('savedMedicines');
            if (info.medicines && info.medicines.length > 0) {
                ...
                html += '</ul>';
                savedMedicines.innerHTML = html;
                savedMedicines.style.display = 'block';
            } else {
                savedMedicines.style.display = 'none';
            }
        }
        ...
        function clearForm() {
            ...
            localStorage.removeItem('lastSavedPatientInfo');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const savedData = localStorage.getItem('lastSavedPatientInfo');
            if (savedData) {
        ...
        async function loadSavedInfo(patientId, visitId) {
            const infoRef = ref(db, `patients/${patientId}/visits/${visitId}/information`);
            const snapshot = await get(infoRef);
            if (snapshot.exists()) {
                const data = snapshot.val();
                displaySavedInfoInForm(data); // Populate the form with the loaded data
            }
        }
        async function saveInfo(patientId, visitId, info) {
            const infoRef = ref(db, `patients/${patientId}/visits/${visitId}/information`);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (patientId && visitId) {
                loadSavedInfo(patientId, visitId);
                
                document.getElementById('saveInfoBtn').addEventListener('click', () => {
                    const info = collectFormData(); // whatever function collects your form data
                    saveInfo(patientId, visitId, info);
                });
            }
        });
