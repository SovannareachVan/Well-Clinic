...
communeInput.addEventListener('input', () => {
    const input = communeInput.value.trim();
    communeList.innerHTML = ''; // Clear old suggestions

    if (!input) return;

    const suggestions = Object.values(addressMapping.commune).filter(commune =>
        commune.startsWith(input)
    );
    ...
    suggestions.forEach(commune => {
        ...
        option.addEventListener('click', () => {
            ...
            communeList.innerHTML = ''; // Clear on select
        });
