...
function initDiagnosisDropdown() {
    ...
    input.addEventListener('input', function () {
        ...
        positionDropdown(input, dropdown, filteredOptions.length);
    });
    ...
    input.addEventListener('click', function () {
        ...
        positionDropdown(input, dropdown, diagnosisOptions.length);
    });
}
...
// Function to position and size the dropdown
function positionDropdown(input, dropdown, itemCount) {
    const rect = input.getBoundingClientRect();
    const itemHeight = 40; // Approximate height of each dropdown-item (adjust based on your styling)
    const desiredHeight = itemCount * itemHeight;

    // Set the dropdown height dynamically, but limit to max-height
    dropdown.style.height = Math.min(desiredHeight, window.innerHeight * 0.8) + 'px'; // Cap at 80% of viewport height
    dropdown.style.maxHeight = '80vh'; // Ensure overflow is handled
    ...
    if (spaceBelow < desiredHeight && spaceAbove > spaceBelow) {
        ...
        dropdown.style.top = (rect.top + window.scrollY - Math.min(desiredHeight, window.innerHeight * 0.8)) + 'px';
    } else {
        // Open downward
...
function initMedicineDropdown(parentElement) {
    ...
    // Show filtered options on input
    input.addEventListener('input', function (e) {
        ...
        if (!scrollHandler && filteredOptions.length) {
            scrollHandler = () => {
        ...
        } else if (!filteredOptions.length && scrollHandler) {
            window.removeEventListener('scroll', scrollHandler);
            scrollHandler = null;
        }
    });

    // Show all options when clicking input
    input.addEventListener('click', function (e) {
        ...
        dropdown.style.display = 'block';
        positionDropdown(input, dropdown, medicineOptions.length);
