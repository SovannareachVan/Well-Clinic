...
async function getPatientDetails(patientId, visitId = null) {
    ...
    try {
        ...
        if (visitId && patientData.visits?.[visitId]) {
            ...
            console.log("Specific visit data for visitId", visitId, ":", visit);
            ...
            visits.forEach(([currentVisitId, visit], index) => {
                console.log(`Visit ${currentVisitId}:`, visit);
...
function generateVisitHtml(title, checkIn, checkOut, clinic, doctor, info, isFirstVisit = false) {
    ...
    console.log("Generating HTML for visit with info:", info);
...
window.onload = function () {
    ...
    const patientId = urlParams.get('patientId'); // Changed to patientId
    const visitId = urlParams.get('visitId');
    console.log('URL Params - patientId:', patientId, 'visitId:', visitId); // Debug log
    if (patientId) {
