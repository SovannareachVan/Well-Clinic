...
const villageNames = {
    village1: "ភូមិ ទួលក្របៅ",
    village2: "ភូមិ គគីរ",
    village3: "ភូមិ ស្នោ"
    // Add all the village names here as needed
};
    ...
    try {
        ...
        if (patientData.address) {
            ...
            if (typeof patientData.address === 'string') {
                // Legacy format - address stored as a single string
                addressString = patientData.address;
            } else {
                // New format - address stored as an object
                const { village, commune, district, province } = patientData.address;
                console.log('Address data:', patientData.address);

                // Map village identifier to full name
                const villageFullName = villageNames[village] || 'N/A';

                // Create address string directly from the values with custom labels
                const addressParts = [
                ...
                addressString = addressParts.join(' '); // Join with space between parts
            }
        ...
        // Continue with your visit handling and other code
