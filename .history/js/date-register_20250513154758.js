...
function checkIn() {
    ...
    const { dateStr, timeStr } = formatDateTime(new Date()); // Use current date and time

    // Serial number
    const serialCell = newRow.insertCell(0);
    serialCell.textContent = rowCount++;

    // Check-in date and time (pre-filled with current date, time auto-generated)
    const checkInCell = newRow.insertCell(1);
    checkInCell.innerHTML = `
        <div class="date-inputs">
    ...
    // Save the new visit to Firebase with the current date-time for checkIn
    const visitData = {
        checkIn: combineDateTime(dateStr.split('/')[0], dateStr.split('/')[1], dateStr.split('/')[2], timeStr),
        checkOut: 'N/A',
