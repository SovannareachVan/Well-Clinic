...
async function getPatientDetails(patientId, visitId = null) {
    try {
        ...
        document.getElementById('totalPriceValue').value = structuredNotes.totalPrice || '0.00'; // Display saved total price
...
// Update savePatientNotes to include total price
async function savePatientNotes(patientId, visitId = null) {
    ...
    const structuredNotes = {
        ...
        medicines,
        totalPrice: document.getElementById('totalPriceValue').value || '0.00', // Save the total price
...
function showTotalPricePopup() {
    try {
        ...
        okButton.onclick = () => {
            ...
            total.value = '$' + selectedTotal.toFixed(2); // Add $ at the end
            popup.remove();
        };
