...
if (patientData.address) {
    ...
    // Extract just the numbers from each address part
    const extractNumber = (str) => {
        const match = str.match(/\d+/);
        return match ? match[0] : '';
    };

    // Check each part of the address and add it only if available
    if (patientData.address.village) {
        const villageNum = extractNumber(patientData.address.village);
        addressParts.push(`ភូមិ ${villageNum}`);
    }
    if (patientData.address.commune) {
        const communeNum = extractNumber(patientData.address.commune);
        addressParts.push(`ឃុំ ${communeNum}`);
    }
    if (patientData.address.district) {
        const districtNum = extractNumber(patientData.address.district);
        addressParts.push(`ស្រុក ${districtNum}`);
    }
    if (patientData.address.province) {
        const provinceNum = extractNumber(patientData.address.province);
        addressParts.push(`ខេត្ត ${provinceNum}`);
    }

    const addressString = addressParts.join(', ');
    updateField('patientAddress', addressString);  // Update the DOM element
}
