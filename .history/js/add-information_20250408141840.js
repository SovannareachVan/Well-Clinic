...
// Assuming you're updating information for a patient
const updatePatientInfo = (patientId) => {
    const patientRef = ref(db, 'patients/' + patientId);

    const updatedData = {
        fullName: document.getElementById('patientFullName').textContent,
        age: document.getElementById('patientAge').textContent,
        gender: document.getElementById('patientGender').textContent,
        phone: document.getElementById('patientPhone').textContent,
        email: document.getElementById('patientEmail').textContent,
        notes: document.getElementById('patientNotes').textContent
    };

    update(patientRef, updatedData)
        .then(() => {
...
// Call the function when the user clicks "Save"
document.getElementById('saveBtn').addEventListener('click', () => {
    const patientId = 'patient123'; // Replace with the actual patient ID
