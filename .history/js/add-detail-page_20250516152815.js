        ...
        <table class="medicine-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td><input type="text" class="total-price-input" readonly onclick="showTotalPriceSelector(this)"></td>
...
// Function to show total price selector and calculate total
function showTotalPriceSelector(totalPriceInput) {
    const medicineItems = document.querySelectorAll('.medicine-item');
    const dropdown = document.createElement('div');
    dropdown.classList.add('total-price-dropdown');
    let selectedTotal = 0;
    ...
    medicineItems.forEach((item, index) => {
        ...
        const div = document.createElement('div');
        div.classList.add('dropdown-item');
        div.textContent = `Medicine ${index + 1}: $${itemTotal.toFixed(2)}`;
        div.onclick = () => {
            selectedTotal += itemTotal;
            document.getElementById('totalPriceValue').value = selectedTotal.toFixed(2);
            dropdown.remove();
        };
        dropdown.appendChild(div);
    });
    ...
    const rect = totalPriceInput.getBoundingClientRect();
    ...
    document.addEventListener('click', function handleOutsideClick(event) {
        if (!dropdown.contains(event.target) && event.target !== totalPriceInput) {
            dropdown.remove();
