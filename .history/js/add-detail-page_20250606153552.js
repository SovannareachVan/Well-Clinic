...
// Function to fetch patient and visit data
async function getPatientDetails(patientId, visitId = null) {
    try {
        ...
        if (visitId) {
            ...
            const [visitSnapshot, visitInfoSnapshot] = await Promise.all([
                get(visitRef),
                get(visitInfoRef)
            ]);
            ...
            if (visitInfoSnapshot.exists()) {
                structuredNotes = visitInfoSnapshot.val();
            }
        } else {
