...
if (patientData.address) {
    ...
    // Fetch the address fields from Firebase
    const { village, commune, district, province } = patientData.address;
    console.log('Address data:', patientData.address);
    
    // Map the fields to the custom labels
    const addressParts = [
        village ? `ភូមិ ${village}` : '', // Village label with value
        commune ? `ឃុំ ${commune}` : '', // Commune label with value
        district ? `ស្រុក ${district}` : '', // District label with value
        province ? `ខេត្ត ${province}` : '' // Province label with value
    ].filter(Boolean); // Remove empty strings

    // Join address parts with a space and display it
    addressString = addressParts.join(' '); // You can use '\n' if you want line breaks instead
